export const MOON_SUN_TICK_COUNT = 18;
...
const getClockTickLabel = (index) => {
  const totalMinutes = Math.round((24 * 60 * index) / MOON_SUN_TICK_COUNT);
...
const getMoonTickLabel = (index) => {
  const cycleProgress = index / MOON_SUN_TICK_COUNT;
...
export const MOON_SUN_RING_DATA = {
  moon: {
    markerGlyph: "moon",
    tickCount: MOON_SUN_TICK_COUNT,
...
export const getMoonSunReadout = (date = new Date()) => {
...
export const getMoonSunRingColors = (paletteName, segmentIndex) => {
  const palettes = {
    moon: [
...
export const getMoonSunMarkerConfig = (paletteName, timestamp = new Date()) => {
  if (paletteName === "moon") {
    return {
      angle: getMoonCycleAngle(timestamp),
      glyph: MOON_SUN_RING_DATA.moon.markerGlyph,
  ...
  return {
    ...
    glyph: MOON_SUN_RING_DATA.sun.markerGlyph,
